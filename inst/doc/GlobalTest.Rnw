...
\date{Package \Rpackage{globaltest}, version
\Sexpr{Biobase::package.version("globaltest")}.\\\ \\
\today}
...
As of version 4.0.0 the default method for calculating the p-value
has changed. To get exactly the same results as in earlier versions,
add the option \Robject{method = "gamma"} to the
\Rfunction{globaltest} function. See section \ref{method} for more
details.
...
the papers (Goeman et al.\ 2004, 2005).
...
interpretations (see Goeman et al.\ 2004, 2005, for more details).
\begin{itemize}
...
dataformat use BioConductor \Rclass{exprSet} input, which is the
standard format for storing gene expression data in BioConductor.
The \Rclass{exprSet} is the preferred input format for data in
\Rpackage{globaletest}, but simple vector/matrix input is also
possible. See \Robject{help(globaltest)}.

\subsection{Leukemia Data}

The first data set is the famous Leukemia data set by Golub et al.
(1999), that are available from the \Rpackage{golubEsets} package on
...
BioConductor. It has 7,129 genes for 72 samples on Affymetrix hu6800
chips. We use \Rpackage{vsn} to normalize the data here, but any
other normalization method may be used instead. Several phenotype
variables are available with \Robject{Golub}, among them
...
Cancer Institute (Van de Vijver et al., 2002), which we have have
included (by kind permission) in the \Rpackage{globaltest} package.
To keep the package small we have restricted the data set to only
407 probes (those probes annotated to 15 pathways contained in our
example annotation \Robject{annotation.vandeVijver}).
...
This loads another \Rclass{exprSet}, called \Robject{vandeVijver},
which contains data of 295 breast cancer patients with 407 selected
genes on quality criteria. The chip was done with Rosetta technology
and the data are already normalized. The response of primary
interest is the survival time, coded in variables ``TIMEsurvival''
(the survival time) and ``EVENTdeath'' (which indicates whether the
observed time indicates a death or a censoring).
...
in BioConductor. These are proved for most commercially available
chips and for many organisms. For example, to get GO and KEGG
annottation for the Leukemia data set, we use the \Rpackage{hu6800}
package.
...
data (here the \Rclass{exprSet} object), the second input
\Rfunarg{Y} should give the response variable. Because the call
above provides an \Rclass{exprSet} for \Rfunarg{X}, we only need to
...
expression measurements (but is may still be an \Rclass{exprSet}).
...
This gives a summary of test and its result. The model was
``logistic'' because the response \Robject{ALL.AML} is two-valued,
and the p-value was calculated using the asymptotic distribution
(see section \ref{method} for details). Below it gives the test
result. The gene set of all genes has 7,129 genes, which were all
included in the test. Then it gives the test statistic $Q$, and its
...
clinical outcome. This means that samples with the same AML/ALL
status tend to have similar expression profiles. It also means that
the expression profile of a non-negligible proportion of the 7,129
genes is differentially expressed between AML and ALL patients. And
it also means that there is potential for prediction of ALL/AML
status from the gene expression profile over all 7,129 genes.
...
ids. Duplicate probe ids are automatically discarded by
...
by the pathway names. For this we need the \Rpackage{KEGG} package,
...
apply the \Rfunction{multtest} function on a collection of pathways
selected on their p-values. For more sophisticated multiple testing
adjustments, see the \Rpackage{multtest} package.
...
A multi-valued response can be tarnsformed to a two-valued response
...
<<golub:Source:factor, eval=FALSE>>=
globaltest(Golub, "factor(Source)")
@

This is especially relevant for time series, as the time points in a
time series should usually be tested with a multinomial model, while
...
<<vandeVijver:survival:alternatives, eval=FALSE>>=
...
The globaltest can calculate the p-value using different methods.
...
based on the asymptotic distribution, using numerical methods of Box
(1954) and Kotz, Johnson and Boyd (1967). This is the recommended
method for large sample sizes. It can be slightly conservative for
small samples. Except in the survival model, p-values below $10^-15$
...
to the asymptotic distribution. This has the advantage that it is
quicker to calculate and that is is less conservative for small
sample sizes. A drawback is that it can be slightly
anti-conservative, especially for large gene sets. There is no gamma
...
statistical details of the model, see Goeman et al.\ (2005).
...
Typically, a significant pathway is a mix op positively and
...
an indication of amount of variation in the response variable that
the confounder explains and of the power lost in the adjustment
process.
...
indicators included in the data: \Robject{StGallen}, \Robject{NIH}
and \Robject{ESR1} (oestrogen receptor status), and
\Robject{Posnodes} (lymph node status):

<<vijver:adjust:all>>=
globaltest(vandeVijver, Surv(TIMEsurvival, EVENTdeath) ~ StGallen + NIH + ESR1 + Posnodes)
@

From the result we see that the microarray data do have additional
predictive value. The p-value did, however, rise substantially
...
with survival. Try \Robject{summary(fit(gt))} for more detailed
output.
...
associated with the response variable. In that case, we can expect
many gene sets, especially the larger ones, also to be associated
with the response.
...
standardized test statistic than cell cycle pathway itself. Pathways
of the same size with a larger standardized test statistic will
almost invariably also have a lower p-value. In this case around
\Sexpr{100*round(result(sampled.gt)[,"Comparative p"],2)} \%\ of 1,000
random `pathways' of size \Sexpr{length(KEGG.cellcycle)} have a larger
standardized test statistic than the Cell Cycle pathway. This
indicates that, although the cell cycle pathway is clearly
differentially expressed between AML and ALL samples (low p-value),
it is not exceptionally significant for a pathway of its size in
this dataset.
...
or \Rfunarg{drawlabels = FALSE} to remove them. Alternatively, one
can store the geneplot to retrieve the numbers or to plot (part of)
the plot later.
...
\centering
<<geneplot:changenames, fig=TRUE, echo=FALSE>>=
plot(myplot)
@
\end{figure}
...
The regression plot plots all pairs of samples,
just like the checkerboard plot, but now showing the covariance
between their clinical outcomes on the X-axis and the covariance
between their gene expression patterns on the Y-axis. The
comparisons of each sample with itself have been excluded.
...
<<regressionplot>>=
gt <- globaltest(Golub, "ALL.AML", kegg)
regressionplot(gt, "04110", sampleid = "1")
@
...
<<regressionplot2>>=
regressionplot(gt, "04110", sampleid = c("1","2"))
@
...
\bibitem[\protect\citeauthoryear{Goeman, Oosting, Cleton-Jansen, Anninga, and
  van Houwelingen}{Goeman et~al.}{2005}]{Goeman2005}
Goeman, J.~J., J.~Oosting, A.~M. Cleton-Jansen, J.~K. Anninga, and J.~C. van
  Houwelingen (2005).
\newblock Testing association of a pathway with survival using gene expression
  data.
\newblock {\em Bioinformatics\/}~{\em 21\/}(9), 1950--1957.

\bibitem[\protect\citeauthoryear{Goeman, van~de Geer, de~Kort, and van
  Houwelingen}{Goeman et~al.}{2004}]{Goeman2004}
Goeman, J.~J., S.~A. van~de Geer, F.~de~Kort, and J.~C. van Houwelingen (2004).
\newblock A global test for groups of genes: testing association with a
  clinical outcome.
\newblock {\em Bioinformatics\/}~{\em 20\/}(1), 93--99.

\bibitem[\protect\citeauthoryear{Golub, Slonim, Tamayo, Huard, Gaasenbeek,
  Mesirov, Coller, Loh, Downing, Caligiuri, Bloomfield, and Lander}{Golub
  et~al.}{1999}]{Golub1999}
Golub, T.~R., D.~K. Slonim, P.~Tamayo, C.~Huard, M.~Gaasenbeek, J.~P. Mesirov,
...
\newblock {\em Science\/}~{\em 286\/}(5439), 531--537.

\bibitem[\protect\citeauthoryear{Van~de Vijver, He, van~'t Veer, Dai, Hart,
  Voskuil, Schreiber, Peterse, Roberts, Marton, Parrish, Atsma, Witteveen,
  Glas, Delahaye, van~der Velde, Bartelink, Rodenhuis, Rutgers, Friend, and
  Bernards}{Van~de Vijver et~al.}{2002}]{Vijver2002}
Van~de Vijver, M.~J., Y.~D. He, L.~J. van~'t Veer, H.~Dai, A.~A.~M. Hart, D.~W.
  Voskuil, G.~J. Schreiber, J.~L. Peterse, C.~Roberts, M.~J. Marton,
  M.~Parrish, D.~Atsma, A.~Witteveen, A.~Glas, L.~Delahaye, T.~van~der Velde,
  H.~Bartelink, S.~Rodenhuis, E.~T. Rutgers, S.~H. Friend, and R.~Bernards
  (2002).
\newblock A gene-expression signature as a predictor of survival in breast
  cancer.
\newblock {\em New England Journal of Medicine\/}~{\em 347\/}(25), 1999--2009.
