...
%\VignetteDepends{globaltest, graphics, vsn, hu6800, golubEsets, KEGG, GOstats, Rgraphviz, multtest}
...
(see Section \ref{references} for full references).
\begin{itemize}
  \item Goeman et al.\ (2004) introduces the principle of global
  testing in microarray data analysis.
  \item Goeman et al.\ (2005) extends the approach to a survival time
  response and explains how to deal with covariates.
  \item Goeman et al.\ (2006) derives the mathematical properties of
...
library(vsn)
data(Golub_Merge)
#TEMPORARY: until vsn supports ExpressionSet: Golub <- vsn(Golub_Merge)
Golub<-Golub_Merge
exprs(Golub)<-exprs(vsn(exprs(Golub_Merge)))
@

This gives us a data set \Robject{Golub}, which is of the format
\Rclass{ExpressionSet}. It has 7,129 genes for 72 samples on Affymetrix
...
annotation for the Leukemia data set, we use the \Rpackage{hu6800}
package.
...
method uses numerical methods of Box (1954) and Kotz, Johnson and
Boyd (1967) for calculating the distribution of non-chi-squared
...
bars are coloured to indicate a positive or a negative association
...
\centering
<<geneplot:changenames2, fig=TRUE, echo=FALSE>>=
plot(myplot)
@
\end{figure}
...
\bibitem[\protect\citeauthoryear{Goeman, van de Geer, and
  van Houwelingen}{Goeman et~al.}{2005}]{Goeman2005}
Goeman, J.~J., S.~A. van de Geer, and J.~C. van
  Houwelingen (2006).
\newblock Testing against a high dimensional alternative.
\newblock {\em Journal of the Royal Statistical Society, Series B\/}~{\em 68\/}(3), 477--493.
