...
which contains data of 295 breast cancer patients with 4,919 genes
preselected on quality criteria. The chip was done with Rosetta
technology and the data are already normalized. The response of
...
\Robject{help(RosettaAnnotation)}), but we will not use it in this
vignette.
