...
First we load all KEGG and GO pathways. This is most easily done
using the various metadata annotation packages that are available on
BioConductor. For the Golub data, which were made using an
Affymetrix hu6800 chip, we use the \Rpackage{hu6800} metadata
package. If no metadata package is available for the chip or
organism used, users can make one with the \Rpackage{AnnBuilder}
package.
...
annotation. Excluding the electronic and unspecified GO-annotations
can be done with
...
The vectors \Robject{cellcycle} (\Sexpr{length(cellcycle)}
probes) and \Robject{cellcycleGO} (\Sexpr{length(cellcycleGO)}
probes) contain the probes on the hu6800 chip that are annotated to
the Cell Cycle pathway. Suppose we are predominantly interested in
this pathway. We want to know whether this group of genes is
associated with the clinical outcome AML versus ALL.

\subsection{Testing the pathway of all genes}
...
The syntax in case of a survival model is as follows. The
\Robject{pData} object of the \Rclass{exprSet} \Robject{X} object should contain
a variable with the survival times (say ``time''), and a variable
with the status (say ``status''), in which 1 indicates that the
subject died and 0 that it was censored. \Rfunction{globaltest} can
then be called with either
...
For more information see the function \Rfunction{Surv} in the
...
Note from the results of the GO cell cycle pathway that
\Rfunction{globaltest} automatically throws away duplicate probe
ids. The vector \Robject{cellcycleGO} is of length
\Sexpr{length(cellcycleGO)}, but only contains
\Sexpr{length(unique(cellcycleGO))} unique probe ids.

We conclude from the test results that the expression pattern of the
cellcycle pathway is notably different between AML and ALL samples.
However, as the test on all genes was significant we can generally
expect most pathways to be significant as well. To get an impression
of how ``special'' this pathway is compared to other pathways in
this data set, one can use the function \Rfunction{sampling}.
...
pathway (\Sexpr{length(cellcycle)} genes) which have a larger
...
\Sexpr{100*round(result(gt.cc)[,"Comparative p"],2)} \%\ of 1,000
random `pathways' of size \Sexpr{length(cellcycle)} have a larger
...
this dataset. By default 1,000 random sets are sampled; this number
can be changed with the option \Rfunarg{ndraws}.

The p-value of globaltest can be calculated using three different
methods, and the user can specify which method to use with the
argument \Rfunarg{method}. Setting \Rfunarg{method} to
``asymptotic'' lets \Rfunction{globaltest} use the asymptotic
distribution of the test statistic $Q$. This is best for large
sample sizes, but the test will be conservative if the small sample
size is small. Alternatively, setting \Rfunarg{method} to ``gamma''
lets \Rfunction{globaltest} use the gamma approximation to the
distribution of the test statistic. The gamma approximation is less
conservative for small samples, but has a tendency to be
anti-conservative in the extreme tails, especially for large
pathways. The third option is \Rfunarg{method} ``permutation'',
which uses permutations of the clinical variable to calculate the
p-value. All possible permutations are used when (in a small data
set) the number of possible permutations is smaller 10,000.
Otherwise 10,000 random permutations are used. The default method is
to use permutations if the number of possible permutations is less
than 10,000 and to use the asymptotic distribution otherwise. Note
that prior to version 4.0.0, the default of \Rfunarg{method} was
``gamma''.

It is also possible to recalculate a p-value with permutations that
has already been calculated with another method. This can be done
with the function \Rfunction{permutations}. This function has an
argument \Rfunarg{nperm}, which can be used to set the number of
...
The permutation p-value is not so accurate in the lower range as it
is always a multiple of one over the number of permutations. It also
has some sampling variance if the permutations are random (as in
this case). The permutations can also be visualized using
\Rfunction{hist} (see below).


\subsection{Adjusting for the presence of covariates}
...
\Rfunarg{sampling}, but (as yet) not with \Rfunarg{permutation}.
...
To correct for multiple testing, the \Rpackage{globaltest} package
contains a function \Rfunction{multtest}, which can calculates
multiplicity-adjusted p-values using either the False Discovery Rate
controlling procedure of Benjamini and Hochberg of the Family-wise
Error rate controlling procedure of Holm. For more sophisticated
multiple testing adjustments, see the \Rpackage{multtest} package.

<<multtest>>=
sort.gt.kegg <- multtest(sort.gt.kegg, "FWER")
sort.gt.kegg[1:5]
@

To make this list more readable, the pathway numbers can be replaced
by the pathway names:
...
For interpretability, the probe identifiers appearing in the plot
can be replaced by the gene symbols:
...
The option \Rfunarg{scale} can be used to rescale the bars to have
unit standard deviation. Alternatively, this can be done later with
...
described under ``geneplot''. The difference is that the option
\Rfunarg{scale} defaults to \Robject{TRUE} in
\Rfunction{sampleplot}.
...
survival' \emph{Bioinformatics} 21 (9) 1950-1957.
\end{itemize}
