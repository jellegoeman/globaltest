...
section.
<<>>=
keggids <- ls(envir = hu6800PATH2PROBE)
keggPW <- mget(keggids, envir = hu6800PATH2PROBE)
...
database, which corresponds to 88 probe sets on the hu6800 chip. Suppose we are 
...
pathway (88 genes) which have a lower p-value than cell cycle 
itself. In this case around 28\%\ of 1,000 random `pathways' of size 88 
