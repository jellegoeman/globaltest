...
on how to interpret a test result, we refer to the papers (Goeman et
al.\ 2004, 2005).
...
To keep the package small we have reduced the data set to only 407
probes (those probes annotated to 15 pathways contained in our
example annotation \Robject{annotation.vandeVijver}) and only 100
random patients.
...
null) the bar exceeds the reference line. Finally the bars are
coloured to indicate a positive or a negative association of the
gene with the response.
...
the points corresponding to the pairs (1,2) up to (1,72). If the
