%\VignetteIndexEntry{Global Test (deprecated version 4)}
%\VignetteDepends{globaltest, vsn, hu6800.db, golubEsets, KEGG.db, multtest}
...
\section*{Deprecated version}

This vignette describes version 4 of the \Rpackage{globaltest} package. As the package has been completely rewritten, as of version 4.99.0 of \Rpackage{globaltest} this vignette and the functions described in it have been deprecated. Please switch to the new implementation based on the \Rfunction{gt} function. The functions described in this vignette will be removed from the package in a later version.

Use of the deprecated \Rfunction{globaltest} will result in a warning. To suppress this warning, use
<<library,echo=FALSE>>=
library(globaltest)
@
<<deprecated>>=
...
\Rfunction{globaltest} function (not recommended). See section \ref{method}
for more details.
